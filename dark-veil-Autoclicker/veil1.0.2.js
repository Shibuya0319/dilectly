...
function clickAtRatio(rx, ry, label) {
  ...
  const x = window.innerWidth * rx;
  const y = window.innerHeight * ry;

  const target = document.elementFromPoint(x, y);

  console.log(`[COORD CLICK] ${label}`, target);

  if (!target) return false;

  return doClick(target, label);
}
...
function chooseAnySkill() {
  ...
  console.log(
    "[SKILL CANDIDATES]",
    candidates.map(el => el.textContent)
  );
  ...
  return doClick(
    candidates[0],
    `スキル選択: ${candidates[0].textContent}`
  );
}
...
function mainLoop() {
  ...
  if (state === "SELECT_JOB") {
    ...
    const target = getAll().find(el =>
      normalize(el.textContent) ===
      normalize(TARGET_JOB)
    );
...
if (state === "CONFIRM_REINCARNATION") {
  ...
  const dialogOpened =
    body.includes("生まれ変わる") ||
    body.includes("元には戻れない");
